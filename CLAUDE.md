...
- **`update-season.yml` をローカルで手動実行するときは `parse-combi` の入力を必ず絞る**。`parse-combi` は (a) 正本を `work/combi.jsonl`(あれば)→ 無ければ `work/combi.jsonl.gz` の順で読み、(b) `cache/combi/` の**全HTMLを**パースして上書きする。CIはチェックアウト直後でキャッシュが空・`combi.jsonl` も無いので「gz正本 + 今回取得した数百件」だけが混ざるが、ローカルには過去の全件キャッシュ(~45,000件、数か月前)と前回実行時の `work/combi.jsonl` が残っているため、そのまま流すと**正本が数か月前の状態へ巻き戻り、CIが後から追加した組が丸ごと消える**(ビルドは警告を出さない)。手順: ① `git checkout HEAD -- scraper/work/combi.jsonl.gz` で正本を最新に ② `rm -f scraper/work/combi.jsonl` ③ 今回クロールした分(`find cache/combi -newermt <実行開始時刻>`)だけを別ディレクトリへ退避して `cache/combi` と入れ替え ④ `parse-combi` → `build` ⑤ キャッシュを戻す。検証は `data/years/{year}.json` のエントリー数が**減っていないこと**(差分更新で消失は起きない)
...
- 決勝得点表の「もしあなたが審査員だったら」(`FinalsScoreTable` + `web/src/lib/myScores.ts`): ユーザーが各組に0〜100を入力できる**仮想の審査員列「あなた」**。実在審査員と同じくチェックボックスで合計に含める/外せて、ヘッダクリックでソートでき、列内順位 `(N)` も出る。**`finals.judges`/`scores` 配列には合成要素を混ぜない**(位置対応が `voters` 照合などbuild側の前提になっているため)— 独立state `youEnabled` + `myScores` で持つ。`enabled[]` に足さないのは `setAll` が `judges.length` 長の配列を作り直すため。保存は localStorage `m1:my-scores:{year}`(キーは `#{combiId}`、全21年の firstRound 全行で combiId は non-null)で、**年ごとに独立・完全にフロント専用**(`data/` も `scraper/` も一切関与しない)。破損JSON/型不正/範囲外/現行データに無いキーは読み込み時に捨てる。「全解除」は実在審査員のみ対象なので1クリックで「あなたの採点だけの順位」になる。入力欄は `type="text"+inputMode="numeric"`(`type="number"` は不正文字で値が消える・ホイールで誤変更されるため)、↑↓で±1点・Enterで**出番順**の次の組へ(合計順の表示は入力のたびに変わるので移動は安定順で行う)・Escで取消
...
- 審査員統計 `data/judges_stats.json` (build_json.py `build_judges_stats`): 名寄せは `overrides/judges.json`。`career`(通算: 採点数・avgDiff=自分の点−その組への個人審査員平均の平均で負=辛口・行内最高/最低点回数(タイは全員計上)・votes/champVotes=最終決戦で優勝者に投票した数。votersマージ済み年のみ分母)/`byYear`(年別採点傾向。2001年の会場票3列は除外。`maxCombis`/`minCombis` = その審査員自身がその年に最も高く/低く評価した組で、同点は全件・出番順、combiId は名寄せできなければ null。フロントは点数と並べて表示し、その年の王者(finalVotes の championCombiId)なら👑を付ける)/`finalVotes`(満場一致・票差。votesから全年分)。**名寄せ未収録の審査員名は警告して生表記のまま集計されるので、新シーズンはビルド警告を見て judges.json へ追記する**
